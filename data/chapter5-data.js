    ...
    ───────────────────────────────────────────── */
    {
      ...
      quiz: [
        ...
        },
        {
          id: "q-ch5-hw01",
          type: "mcq",
          question: "[Homework 4] After running Dijkstra's algorithm at source router A, how is A's FORWARDING TABLE constructed from the results?",
          options: [
            "List every router with its full least-cost path written out",
            "For each destination, record only the NEXT-HOP router — the first hop along the least-cost path from A",
            "Record the distance vector received from each neighbour",
            "Record every link cost in the network"
          ],
          answer: 1,
          explanation: "The forwarding table maps destination → next hop (output port). Dijkstra produces least-cost PATHS; the table keeps only each path's FIRST hop, because forwarding is hop-by-hop — downstream routers make their own decisions. Homework 4 Q11(a): run Dijkstra from A (settle nodes in cost order, tabulating N′, D(v), p(v)), then read off next-hops.",
          xpReward: 25
        }
      ]
    },
    ...
    ───────────────────────────────────────────── */
    {
      ...
      quiz: [
        ...
        },
        {
          id: "q-ch5-hw02",
          type: "calc",
          question: "[Homework 4] Bellman-Ford at router A: A's neighbours are B (link cost 2) and C (link cost 5). B reports its distance to destination X as 4; C reports its distance to X as 1. Compute d_A(X).",
          setup: "Bellman-Ford equation:\nd_A(X) = min over neighbours v { c(A,v) + d_v(X) }\n\nVia B: c(A,B) + d_B(X) = 2 + 4\nVia C: c(A,C) + d_C(X) = 5 + 1",
          answer: 6,
          tolerance: 0,
          unit: "cost",
          calcType: "numeric",
          hint: "min(2+4, 5+1)",
          explanation: "d_A(X) = min(2+4, 5+1) = min(6, 6) = 6 — a tie; either next hop is valid (a real exam would ask you to state both or follow a tie-break rule). The Bellman-Ford equation d_x(y) = min_v{c(x,v) + d_v(y)} is applied per destination to build the whole-network forwarding table of Homework 4 Q11(b) — cost via each neighbour, take the minimum, record the winning next hop.",
          xpReward: 35
        }
      ]
    },
    ...
    ───────────────────────────────────────────── */
    {
      ...
      quiz: [
        ...
        },
        {
          id: "q-ch5-hw03",
          type: "mcq",
          question: "[Homework 4] Why was UDP chosen for SNMP (Simple Network Management Protocol) instead of TCP?",
          options: [
            "UDP provides guaranteed delivery, which management traffic requires",
            "Management traffic must stay lightweight and work even when the network is congested/failing — UDP avoids TCP's connection setup, state, and retransmission overhead",
            "TCP cannot carry SNMP's message format",
            "UDP encrypts management data automatically"
          ],
          answer: 1,
          explanation: "SNMP manages networks that may be in trouble. UDP is connectionless and stateless: no handshake to fail, no per-connection memory on stressed devices, minimal overhead for frequent small polls. Under congestion, TCP's retransmissions would add load exactly when the network can least afford it — the application (SNMP manager) handles any needed retries itself. Homework 4 Q12; the same 'why UDP?' reasoning applies to DNS and DHCP.",
          xpReward: 25
        }
      ]
    }
